 ...
 *  2. Substract background with Rolling Ball Algorithm (Fixed radius if 150)
 *  4. Homogenize by median filter (raidus 2)
 *  5. Thresholdung by local thresholding method ("Phansalkar")
 *  6. Cut overlapping objects using watersheed segmentation
 *  7. Smooth the contour of each particle using the first four fourier descriptors
 *  8. Some objects can overlap again, so do the watersheed segmentation a second time.
 */
...
macro "HAADF" {
	...
	debuglog("Analysd Image: " + imagepath); 
	...
	debuglog("Do irr waterhsed: " + doIrregularWatershed);
	...
	function checkGetNanoDefineDefault(value,NanoDefineDefaultValue){
	 ...
	 function shapefilter(area,areaConvex,solidity,convexity,aspectRatio,minferet, disableScale,fillResultsTable){
		...
		if(disableScale==true){
			//Remove the sacle because the settings are in unitless
	 ...
	 */
	 function applyRollingBall(){
		...
		debuglog("Call Substract Background radius " + segLocalRBR);
	...
	function LocalMethod() {
		...
		if(noDenoise==false){
			...
			debuglog("smoothing facotr " + smoothingFactor);
...
//	getLocationAndSize(wX, wY, wWidth, wHeight);
	...
	if(ellipseFittingMode == 0){
		...
		run("Shape Smoothing", "relative_proportion_fds=5 absolute_number_fds=6 keep=[Relative_proportion of FDs] use black stack");
